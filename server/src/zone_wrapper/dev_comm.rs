...
fn process_json(
    ...
    let msg_type = match &msg["type"] {
        ...
        _ => {
            return Err(MessageFormat::Text(
                "Message field 'type' parsing failure".to_string(),
            ));
        }
    };
...
pub fn parse(
    ...
    match cmd {
        ...
        MessageFormat::Bin(bin) => info!("zone {} received dev bin cmd {:?}", zone.id, bin),
    }
    Some(MessageTarget::Direct(
        MessageFormat::Text("Yes, sir!".to_string()),
        sender.clone(),
    ))
}
